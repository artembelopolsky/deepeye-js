...
let done_model_training = false;

const BASE_URL = "http://127.0.0.1:5000"; // local server (can be flask on Docker)
//const BASE_URL = "http://127.0.0.1:8080/firebase"; // Docker local server
// const BASE_URL = "https://deepeye.labs.vu.nl/firebase";

let DOT_LOCATIONS = [
    {x: 0 + staticDotConfig.dot_diam_max, y: 0 + staticDotConfig.dot_diam_max}, // top left
    {x: window.screen.width/2, y: 0 + staticDotConfig.dot_diam_max}, // top center
    {x:  window.screen.width - staticDotConfig.dot_diam_max, y: 0 + staticDotConfig.dot_diam_max}, // top right
    {x: 0 + staticDotConfig.dot_diam_max, y: window.screen.height/2}, // center left
    {x:  window.screen.width - staticDotConfig.dot_diam_max, y: window.screen.height/2}, // center right
    {x: 0 + staticDotConfig.dot_diam_max, y: window.screen.height-staticDotConfig.dot_diam_max}, // bottom left
    {x: window.screen.width/2, y: window.screen.height-staticDotConfig.dot_diam_max}, // bottom center
    {x:  window.screen.width - staticDotConfig.dot_diam_max, y: window.screen.height-staticDotConfig.dot_diam_max} // bottom right
  ];
...
function uploadData(frame_data, final_batch=false) {
  ...
  fetch(BASE_URL + "/calibrate", {
      ...
      headers: {
          ...
          'type-dataset' : 'train', // training or testing calibration dataset
      ...
      .then(function (json) {
        if(json == 'Done model training') {
          done_model_training = true;
          endExperimentButton.html('Calibration completed.\n Click to continue');
        }
...
function setup(){
  ...
  endExperimentButton = createButton('Calibrating...');
  ...
  shuffle(DOT_LOCATIONS, true); // shuffle static dot location is place
  randomX = window.screen.width/2;
  randomY = window.screen.height/2;
...
function draw(){
  ...
  if(ready){
    ...
    // static dot condition
      ...
      if (base64data.length % 10 == 0 && base64data.length > 0) {
        ...
        if (FrameDataLog.dotNr < (DOT_LOCATIONS.length+1)){
            uploadData(base64data.splice(0, base64data.length));
        }
        else if(FrameDataLog.dotNr == (DOT_LOCATIONS.length+1)){
          requestcounter ++;
          uploadData(base64data.splice(0, base64data.length), final_batch=true); // label as the last batch
        }
      }
      ...
      if(millis() > prevTime + staticDotConfig.dotDuration - deductTime){
        ...
        if(dotVisible){
          ...
          // randomX = random(staticDotConfig.dot_diam_max, window.screen.width-staticDotConfig.dot_diam_max); // pick random dot location, but make sure dot is fully visible on the screen
          // randomY = random(staticDotConfig.dot_diam_max, window.screen.height-staticDotConfig.dot_diam_max);
          if(Math.floor((num_dots+1)/2 -1) < DOT_LOCATIONS.length) {
            randomX = DOT_LOCATIONS[(Math.floor((num_dots+1)/2)) -1].x;
            randomY = DOT_LOCATIONS[(Math.floor((num_dots+1)/2)) -1].y;
          }
  ...
  if(pauze) {
    ...
    print(`Done model training: ${done_model_training}`);
